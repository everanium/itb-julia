...
same; libitb3 builds as `libitb3.dll`. No third-party Julia packages
are required — the binding depends on the `Libdl` stdlib only, and
the tests use the `Test` stdlib.

## Build the shared library

The convenience driver builds `libitb3.so` and resolves + precompiles
...
```julia
opts = Opts()
with_chunk_size!(opts, 65536)
with_wrapper!(opts, false)
...
save_f(sender, "session.blob")               # written by libitb3, mode 0600
...
the caller's own decoder (e.g. `JSON.parse`).
...
- `using LibItb3` exports `load` / `save`, which collide with the
  identically named FileIO.jl entry points in a session that also
  loads FileIO; in that case `import LibItb3 as ITB` and call `ITB.load` /
  `ITB.save` — the alias confines the version to the import line.
...
- FFI calls block the calling Julia thread for their duration; a
  ...
  bytes.
- libitb3 must be reachable at the first FFI call through the lookup
