#!/usr/bin/env bash
#
# run_bench.sh -- micro-benchmark runner for the Julia binding. Builds
# libitb3.so via build.sh, then runs bench/bench.jl: encrypt_message
...
# bench/bench.jl reads ITB_MSG_PROFILE for the Message shape and
...
exec julia --startup-file=no --project=. bench/bench.jl
